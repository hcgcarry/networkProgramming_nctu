...
public:
    ...
    {
        #ifdef debug
        cout << "######filePath " << fileName<< " newStdin " << newStdin << " newstdout " << newStdout << " newstderr " << newStderr << endl;
        #endif
        int childPid = -1;
        ...
        */
        ...
        if (childPid == 0)
        {
            ...
            while(getline(ss,pathItem,':')){
                string filePath = pathItem + "/" + fileName;
                if(execv(filePath.c_str(), argv) <0){
                }
            }
